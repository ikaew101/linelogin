...
export default function DataChiller7Day({ format, data, isLoading, config, saving, configChiller }) {
  ...
  return (
    <div className={`bg-white ${saving != 'JPEG' && 'shadow'} border mx-2 rounded-xl mt-4 `}>
      <div id="tableChiller7Days" className="p-3">
        {/* <div className='md:flex justify-between md:items-center items-start pl-2 pt-2 grid grid-cols-3 '> */}
        <div className='md:flex justify-between items-center  pl-2 pt-2 grid max-md:grid-rows-2'>
          <div className='flex lg:items-center items-start'>
            ...
            {/* <div className='font-bold text-[#828282] ml-3 flex gap-2  max-md:w-[80vw] bg-red-200'> */}
            <div className='font-bold text-[#828282] ml-1 flex flex-wrap gap-2 max-1050:grid max-1050:grid-row-2 max-md:w-[80vw]'>
              <div className="919:flex grid max-919:grid-row-2 gap-1 max-md:flex max-919 :w-max ">
                <div>ข้อมูล Chiller 7 วันย้อนหลัง </div>
                ({rangeDay})
              </div>
          ...
          {/* lg:flex grid max-lg:grid-row-2  */}
          <div className='flex text-xs text-secondary gap-3 items-center sm:justify-end justify-center'>
            {/* <div className='md:flex grid max-md:grid-row-2 text-xs text-secondary gap-1.5 items-center'> */}
            <div className="flex gap-2 justify-end items-center">
              <div className="border grid-rows-2 rounded py-1 text-center">
                เวลาเปิดสาขา
                <p className="text-3xl px-3.5 text-dark font-bold max-sm:text-2xl">
                  {is24h ? "00:00" : config?.openTime && ('0' + (config?.openTime || '0:00')).slice(-5)}
                </p>
              </div>
              <div className="border grid-rows-2 rounded py-1 text-center">
                เวลาปิดสาขา
                <p className="text-3xl px-3.5 text-dark font-bold max-sm:text-2xl">
                  {is24h ? "00:00" : config?.closeTime && ('0' + (config?.closeTime || '0:00')).slice(-5)}
                </p>
              </div>
            </div>
            <svg width="1" height="32" viewBox="0 0 1 32" fill="none" xmlns="http://www.w3.org/2000/svg">
              <line x1="0.5" y1="2.18557e-08" x2="0.499999" y2="32" stroke="#CFD4D9" />
            </svg>

            <div className="flex gap-2 justify-end items-center">
              <div className="border grid-rows-2 rounded py-1 text-center bg-white px-2.5">
                เวลาเปิดโดยเฉลี่ย
                <p className="text-3xl px-3.5 text-dark font-bold max-sm:text-2xl">
                  {config?.avgOpenTime7 ? ('0' + (config?.avgOpenTime7)).slice(-5) : '-'}
                </p>
              </div>
              <div className="border grid-rows-2 rounded py-1 text-center bg-white px-2.5">
                เวลาปิดโดยเฉลี่ย
                <p className="text-3xl px-3.5 text-dark font-bold max-sm:text-2xl">
                  {config?.avgCloseTime7 ? ('0' + (config?.avgCloseTime7)).slice(-5) : '-'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid mt-2">
          <div className="scrollbar overflow-x-auto">
            <table className="min-w-[1010px] text-sm w-full table-fixed">
              ...
              <tbody>
                {/* {data?.sort((a, b) => new Date(b.lastUpdate) - new Date(a.lastUpdate))?.map((ele, ind) => { */}
                {data?.sort((a, b) => a.seq - b.seq)?.map((ele, ind) => {
                  return (ind < perPage * page && ind + 1 > perPage * page - perPage) &&
                    <tr className="odd:bg-white even:bg-table text-center text-dark rounded-md" key={ind}>
                      ...
                      <td colSpan={2} className="p-2">
                        {/* {ele.activeTime ?
                          Math.floor(ele.activeTime / 60).toString().padStart(2, '0') + ':' + (ele.activeTime % 60).toString().padStart(2, '0')
                          : '-'} */}
                        {Math.floor(ele.activeTime / 60).toString().padStart(2, '0') + ':' + (ele.activeTime % 60).toString().padStart(2, '0')}
                      </td>
                      <td colSpan={2}>
                      ...
                      <td colSpan={2}>
                        <div className={`${ele.setpoint == '-999' ? '' :
                          ele.type == 'Normal' ? (ele.setpoint < configChiller.normal.setpoint && classRed)
                            : (ele.setpoint < configChiller.LFLT.setpoint && classRed)}`}>
                          {ele.setpoint == '-999' ? '-' : ele.setpoint.toFixed(2)}
                        </div>
                      </td>
                      <td colSpan={3}>
                        <div className={`${ele.Approach_Evap_Cooler == '-999' ? '' :
                          ele.type == 'Normal' ? (ele.Approach_Evap_Cooler < configChiller.normal.condApproach.min ? classBlue : ele.Approach_Evap_Cooler > configChiller.normal.condApproach.max && classRed)
                      ...
                      <td colSpan={3}>
                        <div className={`${ele.Leaving_Water_Temp_Cooler == '-999' ? '' :
                          ele.type == 'Normal' ? (ele.Leaving_Water_Temp_Cooler < configChiller.normal.leavingChilledWater.min ? classBlue : ele.Leaving_Water_Temp_Cooler > configChiller.normal.leavingChilledWater.max && classRed)
                      ...
                      <td colSpan={3}>
                        <div className={`${ele.Entering_Water_Temp_Cooler == '-999' ? '' :
                          ele.type == 'Normal' ? (ele.Entering_Water_Temp_Cooler < configChiller.normal.enteringChilledWater.min ? classBlue : ele.Entering_Water_Temp_Cooler > configChiller.normal.enteringChilledWater.max && classRed)
                      ...
                      <td colSpan={3}>
                        <div className={`${ele.Delta_Cooler == '-999' ? '' :
                          ele.type == 'Normal' ? (ele.Delta_Cooler < configChiller.normal.averageCoolerTemp.min ? classBlue : ele.Delta_Cooler > configChiller.normal.averageCoolerTemp.max && classRed)
                      ...
                      <td colSpan={3}>
                        <div className={`${ele.Approach_Conderser == '-999' ? '' :
                          ele.type == 'Normal' ? (ele.Approach_Conderser < configChiller.normal.condApproach.min ? classBlue : ele.Approach_Conderser > configChiller.normal.condApproach.max && classRed)
                      ...
                      <td colSpan={3}>
                        <div className={`${ele.Leaving_Water_Temp_Conderser == '-999' ? '' :
                          ele.type == 'Normal' ? (ele.Leaving_Water_Temp_Conderser < configChiller.normal.leavingCondWater.min ? classBlue : ele.Leaving_Water_Temp_Conderser > configChiller.normal.leavingCondWater.max && classRed)
                      ...
                      <td colSpan={3}>
                        <div className={`${ele.Entering_Water_Temp_Conderser == '-999' ? '' :
                          ele.type == 'Normal' ? ele.Entering_Water_Temp_Conderser < configChiller.normal.enteringCondWater.min ? classBlue : ele.Entering_Water_Temp_Conderser < configChiller.normal.enteringCondWater.max && classRed
                      ...
                      <td colSpan={3} className="rounded-r-md">
                        <div className={`${ele.Delta_Conderser == '-999' ? '' :
                          ele.type == 'Normal' ? ele.Delta_Conderser < configChiller.normal.averageCondTemp.min ? classBlue : ele.Delta_Conderser < configChiller.normal.averageCondTemp.max && classRed
      ...
      <div className={`${data?.length && "sm:absolute sm:-mt-10"}`}>
        <ExportTypes id="tableChiller7Days"
          csvData={data?.map((ele, i) => {
            const objThai = {
              ลำดับ: ele.seq,
              ยี่ห้อ: ele.chillerBandName,
              ประเภท: ele.type,
              // 'Active time (h:mm)': ele.activeTime ? Math.floor(ele.activeTime / 60).toString().padStart(2, '0') + ':' + (ele.activeTime % 60).toString().padStart(2, '0') : '-',
              'Active time (h:mm)': Math.floor(ele.activeTime / 60).toString().padStart(2, '0') + ':' + (ele.activeTime % 60).toString().padStart(2, '0'),
              ...
              'AVG Leaving Water temp Cooler (°F)': ele.Leaving_Water_Temp_Cooler.toFixed(2),
              'AVG Entering Water temp Cooler (°F)': ele.Entering_Water_Temp_Cooler.toFixed(2),
              'Delta Cooler (°F)': ele.Delta_Cooler.toFixed(2),
              'AVG Approach Condenser (°F)': ele.Approach_Conderser.toFixed(2),
              'AVG Leaving Water temp Condenser (°F)': ele.Leaving_Water_Temp_Conderser.toFixed(2),
              'AVG Entering Water temp Condenser (°F)': ele.Entering_Water_Temp_Conderser.toFixed(2),
