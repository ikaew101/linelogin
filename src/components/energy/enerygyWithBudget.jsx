...
function EnerygyWithBudget({ ranks, isLoading, isMini, storeData, saving }) {
    const [perPage, setPerPage] = useState(10)
    const [page, setPage] = useState(1)
    // const lastPage = Math.round(ranks?.length / perPage) || 1
    ...
    useEffect(() => {
        // ranks?.map(ele => {
        //     ele.diff = ele.total - ele.budget
        //     ele.percentDiff = (ele.total - ele.budget) / ele.total * 100
        // })
        const data = (ranks?.filter(ele => !!ele.total))?.sort((b, a) => a.actualVsBudget - b.actualVsBudget)
    ...
    // ?.filter(ele => !!ele.total))?.sort((b, a) => a.actualVsBudget - b.actualVsBudget)
    //     .concat((data?.filter(ele => !ele.total))?.sort((a, b) => b.total - a.total))?.map((ele, i) => {
    //         return { ...ele, index: i + 1 }
    //     })
    ...
    return (
        <div className={`bg-white ${saving != 'JPEG' ? 'shadow' : ''} border mx-2 rounded-xl mt-4 text-secondary`}>
            <div id={`EnergyWithBudget`} className="p-3 pb-5">
                <div className="flex items-center">
                    <img src="icon/table.png" className="w-10 h-10" />
                    <p className="py-5 text-lg font-bold pl-2" id="header-text">การใช้พลังงานสูงสุดเมื่อเทียบกับงบประมาณ</p>
                </div>
                ...
                <div id="table" className="grid">
                    <div className={`overflow-x-auto scrollbar ${!isLoading && data && perPage >= 10 && "min-h-[500px]"}`}>
                        <table width="100%" className="min-w-[900px] text-sm w-full">
                                ...
                                <tr>
                                    ...
                                    <th onClick={sortData} >
                                        <div className={`flex gap-0.5 cursor-pointer justify-center items-center`} id="other">
                                            ...
                                            <div className="flex self-center h-min">
                                                <img id="budget" className={`self-end ${sort?.budget == 'desc' && "h-[14px]"} w-[4px]`} src='img/sortUp.svg' />
                                                <img id="budget" className={`self-end ${sort?.budget == 'asc' && "h-[14px]"} w-[4px]`} src='img/sortDown.svg' />
                                            </div>
                                        </div>
                                    </th>
                                    <th onClick={sortData} >
                                        <div className={`flex gap-0.5 cursor-pointer justify-center items-center`} id="other">
                                            <div id="actualVsBudget">
                                                <p id="actualVsBudget">ส่วนต่างจากงบประมาณ</p>
                                                (kWh)
                                            </div>
                                            <div className="flex self-center h-min">
                                                <img id="actualVsBudget" className={`self-end ${sort?.actualVsBudget == 'desc' && "h-[14px]"} w-[4px]`} src='img/sortUp.svg' />
                                                <img id="actualVsBudget" className={`self-end ${sort?.actualVsBudget == 'asc' && "h-[14px]"} w-[4px]`} src='img/sortDown.svg' />
                                            </div>
                                        </div>
                                    </th>
                                    <th onClick={sortData} className="rounded-r-md py-2">
                                        <div className={`flex gap-0.5 cursor-pointer justify-center items-center`} id="other">
                                            <div id="actualVsBudgetPercent">
                                                <p id="actualVsBudgetPercent">ส่วนต่างจากงบประมาณ</p>
                                                (%)
                                            </div>
                                            <div className="flex self-center h-min pr-1">
                                                <img id="actualVsBudgetPercent" className={`self-end ${sort?.actualVsBudgetPercent == 'desc' && "h-[14px]"} w-[4px]`} src='img/sortUp.svg' />
                                                <img id="actualVsBudgetPercent" className={`self-end ${sort?.actualVsBudgetPercent == 'asc' && "h-[14px]"} w-[4px]`} src='img/sortDown.svg' />
                                            </div>
                                        </div>
                                    </th>
                                    {/* <th>
                            ...
                            <th className="py-2 max-w-[5.5em]">ส่วนต่างจากงบประมาณ (kWh)</th>
                            <th className="rounded-r-md max-w-[5.5em]">ส่วนต่างจากงบประมาณ (%)</th> */}
                                </tr>
                            </thead>
                            {!isLoading && dataSorted?.length &&
                                <tbody>
                                    <tr className="bg-[#E3F2FE] text-center rounded-md" hidden={!dataSorted}>
                                        {/* <td className="rounded-l-md">{myRank.index || '-'}</td> */}
                                        <td className="rounded-l-md">
                                            {/* {data?.findIndex(ele => ele.storeId == storeData.id) >= 0 ? data?.findIndex(ele => ele.storeId == storeData.id) + 1 : '-'} */}
                                            {
                                                // (sort?.index || sort?.actualVsBudget) ? 
                                                myRank?.index || '-'
                                                // : dataSorted?.findIndex(ele => ele.storeId == storeData.id) >= 0
                                                //     ? dataSorted?.findIndex(ele => ele.storeId == storeData.id) + 1 : '-'
                                            }
                                            {/* {myRank?.index || '-'} */}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            {myRank?.budget?.toLocaleString('en-US', optionDigit) || '-'}
                                        </td>
                                        <td>
                                            {/* {myRank.total && myRank.budget ? (myRank.total - myRank.budget)?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'} */}
                                            {myRank?.actualVsBudget ? (myRank?.actualVsBudget)?.toLocaleString('en-US', optionDigit) : '-'}
                                        </td>
                                        <td className="rounded-r-md">
                                            <div className={`whitespace-nowrap
                                        w-fit mx-auto p-1.5 rounded-md my-2 px-2 w-[80%] max-w-[5em]`}
                                            >
                                                {myRank?.actualVsBudgetPercent
                                                    ? `${(myRank.actualVsBudgetPercent)?.toLocaleString('en-US', optionDigit) || ""}%`
                                                    : '-'}
                                            </div>
                                        </td>
                                    </tr>

                                    {dataSorted?.map((ele, index) => (
                                        index < perPage * page && index + 1 > perPage * page - (perPage) &&
                                        <tr key={index} className={`odd:bg-white even:bg-table text-center rounded-md`}>
                                            ...
                                            <td>
                                                {ele.budget?.toLocaleString('en-US', optionDigit) || '-'}
                                            </td>
                                            <td>
                                                {ele.actualVsBudget ? (ele.actualVsBudget)?.toLocaleString('en-US', optionDigit)
                                                    // : ele.actualVsBudget}
                                                    : "-"}
                                                {/* {(ele.diff)?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} */}
                                            </td>
                                            <td className="rounded-r-md">
                                                <div className="p-1.5 rounded-md  my-2 px-2"
                                                // className={`${ele.budget ?
                                                //     (ele.actualVsBudget / ele.budget * 100) < 0 ? "bg-danger text-white" : "bg-primary text-white"
                                                //     : ""} w-fit mx-auto p-2 rounded-md  my-1 px-2.5 w-[80%] max-w-[5em]`}
                                                >
                                                    {ele.actualVsBudgetPercent ?
                                                        // `${(ele.actualVsBudget / ele.budget * 100)?.toLocaleString('en-US', optionDigit)}%`
                                                        `${(ele.actualVsBudgetPercent)?.toLocaleString('en-US', optionDigit) || ""}%`
                                                        : "-"}
                                                </div>
                                            </td>
                                        </tr>
                    ...
                    : !data?.length
                        ...
                        : <>
                            ...
                            <div className="sm:absolute sm:-mt-8">
                                <ExportTypes id={`tableEnergyWithBudget`}
                                    csvData={data.map((ele, i) => {
                                        const objThai = {
                                            ...
                                            'งบประมาณ (kWh)': ele.budget?.toFixed(2) || "",
                                            'ส่วนต่างจากงบประมาณ (kWh)': ele.actualVsBudget?.toFixed(2) || "",
                                            'ส่วนต่างจากงบประมาณ (%)': ele.actualVsBudgetPercent?.toFixed(2) || "",
                                        }
                                        if (isMini) {
            ...
            <div className="h-0 overflow-hidden">
                <div id={`tableEnergyWithBudget`} className="px-3 ">
                    ...
                    <div id="table" className="grid">
                        <div className={`overflow-x-auto scrollbar max-h-[80vh] ${!isLoading && data && perPage >= 10 && "min-h-[500px]"}`}>
                            <table width="100%" className="min-w-[900px] text-sm w-full">
                                ...
                                {!isLoading && dataSorted?.length &&
                                    <tbody>
                                        <tr id='trow_0' className="bg-[#E3F2FE] text-center rounded-md" hidden={!dataSorted}>
                                            <td className="rounded-l-md">
                                                {myRank?.index || '-'}
                                            </td>
                                            <td>
                                        ...
                                        {dataSorted?.map((ele, index) => (
                                            index < perPage * page && index + 1 > perPage * page - (perPage) &&
                                            <tr id={`trow_${index + 1}`} key={index} className={`odd:bg-white even:bg-table text-center rounded-md`}>
                                                <td className="rounded-l-md">
                                                    {ele.index}
                                                </td>
                                                <td>
...
export default EnerygyWithBudget