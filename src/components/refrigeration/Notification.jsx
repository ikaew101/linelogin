...
function NotificationPage({ userId, selectedStore, setSelectedStore, stores }) {
    ...
    const [toggleWork, setToggleWork] = useState(false || searchParams.get("toggleWork") === 'true')
    ...
    const getData = async () => {
        ...
        try {
            ...
            const res = await RefrigService.getRefrigNotificationById(userId)
    ...
    return (
        <>
            <div className={`bg-white border ${saving != 'JPEG' && 'shadow'} -mt-10 mx-2 rounded-xl`}>
                <div id="storeData" className="p-3">
                    ...
                    <div className="xs:flex justify-end gap-5 items-center text-sm px-2 pr-5 p-2 max-xs:ml-2">
                        <div className="flex max-xs:pb-2">
                            {/* <label className="relative inline items-center cursor-pointer">
                                <input type="checkbox" checked={toggleAll} className="sr-only peer"
                                    onChange={() => setToggleAll(!toggleAll)}
                                />
                                <div className="peer-checked:bg-primary w-11 h-6 bg-gray-200 peer-focus:outline-none  peer-focus:ring-blue-300  rounded-full peer  peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all mr-2" />
                            </label> */}
                        ...
                        <div className="flex">
                            {/* <label className="relative inline items-center cursor-pointer">
                                <input type="checkbox" checked={toggleWork} className="sr-only peer"
                ...
                <div className="md:absolute md:-mt-9 mt-2">
                    <ExportPage onSaving={setSaving} name="Refrig Noti"
                ...
                <div id="top">
                    ...
                    <div className='bg-primary p-2 my-4 mx-6 lg:mx-10 rounded-md justify-center flex lg:gap-14 relative z-30 gap-1 max-lg:grid max-lg:grid-cols-3 font-medium'>
                        ...
                        <button name='notification'
                            className={`bg-[#009B93] py-2  rounded-md text-white max-lg:w-full lg:min-w-[250px]    px-2 transition-colors duration-100 `}
                        >
                            การแจ้งเตือน
                        </button>
                    </div>
            ...
            <Table isLoading={isLoading} data={data} toggleAll={toggleAll} toggleWork={toggleWork} storeNo={storeData.storeNumber} setIsLoading={setIsLoading}
                page={page} perPage={perPage} setPage={setPage} setPerPage={setPerPage}
                saving={saving} />
...
function Table({ isLoading, data, toggleAll, toggleWork, storeNo, setIsLoading, callbackLengthDataShow, saving, page, perPage, setPage, setPerPage }) {
    ...
    function downloadCSV() {
        ...
        dataSorted.map(ele => {
            dataExport.push({
                ...
                "adr": ele.infoAdr,
                "ชื่ออุปกรณ์": ele.infoName,
                "หมวดหมู่": ele.category,
                "เวลาที่เกิด alarm (ครั้งแรก)": new Date(null).toLocaleString() !== new Date(ele.timestampFirstTime).toLocaleString() ? ele.timestampFirstTime : "",
                "เวลาที่เกิด alarm (ล่าสุด)": new Date(null).toLocaleString() !== new Date(ele.timestampCurrent).toLocaleString() ? ele.timestampCurrent : "",
                "เปิดคำสั่งงาน": ele.OpenWorkOrder,
                "เวลาที่เปิดคำสั่งงาน": new Date(null).toLocaleString() !== new Date(ele.timestampOpenWorkOrder).toLocaleString() ? ele.timestampOpenWorkOrder : "",
                "ระยะเวลาที่เกิด alarm": ele.durationAlarm,
            })
        })
    ...
    return (
        <div className={`bg-white border ${saving != 'JPEG' && 'shadow'} mx-2 rounded-xl mt-4 p-3 text-secondary`}>
            {/* <div className="flex justify-end mb-4 hidden" id={'title'}>
            </div> */}

            <div className="grid p-2" id="tableNotification">
                    ...
                    <table width="100%" className="min-w-full table-fixed max-lg:min-w-[950px]">
                        ...
                        <thead className="text-dark bg-table-head gap-0.5 text-sm rounded-md"
                            ...
                            <tr>
                                ...
                                <th rowSpan="2" className="" >
                                    <div className="flex mx-1 cursor-pointer justify-center group" onClick={toggleSort} id="infoName">
                                        ชื่ออุปกรณ์
                                        <div className="flex self-center h-min ml-1">
                                            <img id="infoName" className={`self-end ${sort?.infoName == 'desc' && "h-[14px]"} w-[4px]`} src='img/sortUp.svg' />
                                            <img id="infoName" className={`self-end ${sort?.infoName == 'asc' && "h-[14px]"} w-[4px]`} src='img/sortDown.svg' />
                                        </div>
                                    </div>
                                </th>
                                <th rowSpan="2" className="">
                        ...
                        <tbody className={`text-[14px] `}>
                            ...
                            {dataSorted?.map((ele, ind) => {
                                ...
                                if (ind < perPage * page && ind + 1 > perPage * page - perPage)
                                    return (
                                        <tr id={`trow_${ind}`}
                                            key={ind} className="odd:bg-white even:bg-table text-center rounded-md">
                                            ...
                                            <td >{ele.infoAdr}</td>
                                            <td className="text-left">{ele.infoName}</td>
                                            <td >
            ...
            <div className="">
                ...
                <div className={`${!isLoading && dataSorted?.length ? 'sm:absolute sm:-mt-8' : ''}   -ml-3 `}>
